...
const mockFetchCryptoByExchanges = () => {
  CoinGeckoProviderStub.fetchCryptoByExchanges.withArgs(sinon.match.any, sinon.match.any, sinon.match(2)).resolves({
    data: {
      tickers: []
    }
  });

  CoinGeckoProviderStub.fetchCryptoByExchanges.withArgs(sinon.match.any, sinon.match.any, sinon.match(1)).resolves({
    data: {
      tickers: [
        {
          base: "BTC",
